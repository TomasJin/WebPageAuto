...
class LoginUpwork:
    ...
    def act_on_login_page(self):
        self.bid = self.bid == False
        self.proposal = 50
        if self.bid:
            email = helper.ensure_find_element(self.driver, By.XPATH, './/input')
            with open('dataout.txt', "r+") as file:
                lines = file.readlines()
            line = lines[0]
            date_last = line[line.find('+') + 1: line.find('+') + 15]
            date_obj_last = datetime.strptime(date_last, "%Y%m%d%H%M%S")
            date_now = datetime.now()
            diff = date_now - date_obj_last
            if diff.days < 2:
                return
            lines = lines[1:]
            ...
            self.fake_address = line
            helper.ensure_click(self.driver, By.ID, 'login_password_continue')
            time.sleep(5)
            password = helper.ensure_find_element(self.driver, By.ID, 'login_password')
            password.send_keys(self.data['password'])
            helper.ensure_click(self.driver, By.ID, 'login_control_continue')
            time.sleep(10)
        else:
    ...
    def act_on_resume_import_page(self):
        ...
        try:
            helper.ensure_find_element(self.driver, By.XPATH, '//input[@type="file"]').send_keys(
                constant.CWD + '//' + str(self.num) + self.data["RESUME_ADDRESS"])
            time.sleep(10)
        except:
            pass
    ...
    def act_on_education_page(self):
        ...
        if page_id == EDUCATION_PAGE:
            helper.ensure_click(self.driver, By.XPATH, '//button[@data-test="skip-button"]')

    def act_on_certification_page(self):
        if self.data["CERTIFICATION"]:
            helper.ensure_click(self.driver, By.XPATH, "//button[@data-qa='certifications-add-btn']")
            helper.ensure_click(self.driver, By.XPATH, "//div[@class='up-loader-container']")
            element = helper.ensure_find_element(self.driver, By.CSS_SELECTOR, 'div.air3-modal-body')
            element = helper.ensure_find_element(element, By.XPATH, './/input')
            try:
                element.send_keys(self.data["CERTIFICATION"])
            except:
                pass
            element = helper.ensure_find_element(self.driver, By.CSS_SELECTOR,
                                                 'div[tabindex="-1"].air3-dropdown-menu-container')
            time.sleep(2)
            helper.ensure_click(element, By.XPATH, './/li')
            helper.ensure_click(self.driver, By.XPATH, '//button[@data-qa="btn-save"]')
            time.sleep(3)
            helper.ensure_click(self.driver, By.XPATH, "//button[@data-test='next-button']")
        else:
            helper.ensure_click(self.driver, By.XPATH, '//button[@data-test="skip-button"]')
        page_id = self.recognize_page()
        if page_id == CERTIFICATION_PAGE:
            helper.ensure_click(self.driver, By.XPATH, '//button[@data-test="skip-button"]')
    ...
    def act_on_location_page(self):
        ...
        try:
            helper.ensure_find_element(self.driver, By.XPATH, '//input[@type="file"]').send_keys(
                constant.CWD + '//' + str(self.num) + self.data["PHOTO_ADDRESS"])
    ...
    def act_on_best_match_page(self):
        ...
        if self.proposal == 50:
            ...
            #                     'button[type="button"][role="tab"][data-test="tab-most-recent"]')
            time.sleep(5)
            helper.execute_with_retry(self.driver, self.driver.get(
                "https://www.upwork.com/nx/jobs/search/?q=%28" + data["MY_SKILLS"][0] +
            ...
            element = helper.ensure_find_element(self.driver, By.XPATH, '//nav/ul/li[10]')
            helper.ensure_click(element, By.TAG_NAME, "button")
            helper.ensure_click(element, By.XPATH, "./ul/li[4]/ul/li[2]/button")
    ...
    def act_on_proposal_page(self):
        ...
        # time.sleep(5)
        if self.act_on_proposal_modal() == "No_Bid":
            return
        time.sleep(3)
        if self.act_on_proposal_modal() == "No_Bid":
            return
        ...
        if current_url.find(constant.PROPOSAL_URL) == -1:
            with open('data_bid.txt', "a+") as file:
                file.write(self.fake_address)
        else:
            with open('dataout.txt', "r+") as file:
                lines = file.readlines()
    ...
    def act_on_duration(self):
        duration = helper.ensure_find_element(self.driver, By.XPATH,
    ...
    def act_on_bid_additional_detail(self):
        additional_details = helper.ensure_find_element(self.driver, By.XPATH,
                                                        '//div[@class="fe-proposal-additional-details additonal-details"]')
    ...
    def act_on_proposal_modal(self):
        ...
        if modal and modal.text.find("Buy Connects") == 0:
            self.driver.execute_script("window.close()")
            self.driver.switch_to.window(self.driver.window_handles[0])
            time.sleep(2)
            self.driver.get(constant.BEST_MATCH_URL)
